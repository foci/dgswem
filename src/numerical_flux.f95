...
!$$$
!$$$      GX_IN = (QX_IN*QX_IN/(HT_IN) 
!$$$     &     + 0.5D0*G*(HT_IN**2))*SFAC_IN 
!$$$      !&     - (HT_IN*bed_IN(1)*G)*SFAC_IN 
!$$$      GY_IN = QX_IN*QY_IN/(HT_IN)
!$$$
!$$$C.....Compute y momentum fluxes at interior state
!$$$
!$$$      HX_IN = (QX_IN*QY_IN/(HT_IN))*SFAC_IN
!$$$      HY_IN = (QY_IN*QY_IN/(HT_IN) +
!$$$     &     0.5D0*G*(HT_IN**2))
!$$$      !&     - (HT_IN*bed_IN(1)*G)
!$$$
!$$$C.....Compute x momentum fluxes at exterior state
!$$$
!$$$      GX_EX = (QX_EX*QX_EX/(HT_EX) 
!$$$     &     + 0.5D0*G*(HT_EX**2))*SFAC_EX
!$$$      !&     - (HT_EX*bed_EX(1)*G)*SFAC_EX
!$$$      GY_EX = QX_EX*QY_EX/(HT_EX)
!$$$
!$$$C.....Compute y momentum fluxes at exterior state
!$$$
!$$$      HX_EX = (QX_EX*QY_EX/(HT_EX))*SFAC_EX
!$$$      HY_EX = (QY_EX*QY_EX/(HT_EX) +
!$$$     &     0.5D0*G*(HT_EX**2))
!$$$      !&     - (HT_EX*bed_EX(1)*G)
!$$$
!$$$
...
!$$$            if(porosity*((3*QX_ROE**2 + QX_ROE*QY_ROE + QY_ROE**2/2)/(ZE_ROE + bed_ROE)**3).ne.dtildeQx(w)) then
!$$$               write(*,*)'That was foolish on timestep', it
!$$$               WRITE(*,*)'*** Error: ',EvalErrMsg ()
!$$$               WRITE(*,*)'*** Error: ',EvalErrMsg2 ()
!$$$               WRITE(*,*) funcx(w),'X=',dtildeQx(w),w
!$$$               WRITE(*,*) funcy(w),'Y=',dtildeQy(w),w
!$$$               WRITE(*,*) 'valx = ', valx(w), w
!$$$               WRITE(*,*) 'valy = ', valy(w), w
!$$$               print*, '((3*QX_ROE**2 + QX_ROE*QY_ROE + QY_ROE**2/2)/(ZE_ROE + bed_ROE)**3)
!$$$     &              actualX=', porosity*((3*QX_ROE**2 + QX_ROE*QY_ROE + QY_ROE**2/2)/(ZE_ROE + bed_ROE)**3)
!$$$               print*, '(QX_ROE*QY_ROE + QY_ROE**2/2)/(ZE_ROE + bed_ROE)**3  
!$$$     &              actualY=', porosity*((QX_ROE*QY_ROE + QY_ROE**2/2)/(ZE_ROE + bed_ROE)**3 ) 
...
!$$$      lambda3_EX = 0.D0
!$$$
!$$$      lambda2_IN = (U_IN*NX+V_IN*NY)
!$$$      lambda2_EX = (U_EX*NX+V_EX*NY)
!$$$
...
!$$$      lambda4_EX = 0.D0
!$$$
!$$$      lambda4_IN = (-3.D0*QX_IN/(ZE_IN + bed_IN(1))**4 + (ZE_IN + bed_IN(1))**(-3)
!$$$     &     -3.D0*QX_IN/(ZE_IN + bed_IN(1))**4)*QMag_IN*NX 
!$$$     &     + (-3.D0*QY_IN/(ZE_IN + bed_IN(1))**4 + (ZE_IN + bed_IN(1))**(-3)
!$$$     &     -3.D0*QY_IN/(ZE_IN + bed_IN(1))**4)*QMag_IN*NY 
!$$$
!$$$      lambda4_EX = (-3.D0*QX_EX/(ZE_EX + bed_EX(1))**4 + (ZE_EX + bed_EX(1))**(-3)
!$$$     &     -3.D0*QX_EX/(ZE_EX + bed_EX(1))**4)*QMag_EX*NX 
!$$$     &     + (-3.D0*QY_EX/(ZE_EX + bed_EX(1))**4 + (ZE_EX + bed_EX(1))**(-3)
!$$$     &     -3.D0*QY_EX/(ZE_EX + bed_EX(1))**4)*QMag_EX*NY 
!$$$
!$$$
!$$$
