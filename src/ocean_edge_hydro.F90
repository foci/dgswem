...
!.....Compute dg_here%ZE, dg_here%QX, dg_here%QY, and dg_here%HB at each global_here%edge Gauss quadrature point
         ...
         DO I = 1,dg_here%NEGP(dg_here%pa)
            ...
            dg_here%QX_EX = 0.D0
            dg_here%QY_EX = 0.D0
            global_here%U_EX  = 0.D0
            global_here%V_EX  = 0.D0
...
#ifdef SED_LAY 
            dg_here%HB(:,global_here%EL_IN,dg_here%irk) = 0.D0
            do ll=1,s%layers
...
!.....Compute the specified open ocean elevation
            ...
            DO JJ=1,global_here%NBFR
               
               IF (global_here%PER(JJ).EQ.0.D0) THEN
                  global_here%NCYC = 0.D0
               ELSE
                  global_here%NCYC = INT(dg_here%TIMEDG/global_here%PER(JJ))
               ENDIF
               
!...........Surface Elevation

               global_here%ARGJ = global_here%AMIG(JJ)*(dg_here%TIMEDG - global_here%NCYC*global_here%PER(JJ)) + global_here%FACE(JJ)
               global_here%RFF = global_here%FF(JJ)*dg_here%RAMPDG
               ...
               global_here%ARG = global_here%ARGJ - dg_here%EFA_GP
               
               dg_here%ZE_EX = dg_here%ZE_EX + dg_here%EMO_GP*global_here%RFF*COS(global_here%ARG) 
