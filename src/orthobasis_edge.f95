            ...
            DO A=0,2*P+2
               DO B=0,1
                  DO J=0,P
                     ...
                     ELSEIF (J.EQ.1) THEN
                        dg%JACOBI(J+1,A+1,B+1,Q) =(2*(A+1)+(A+B+2)*(Z-1.D0))/2.D0
                     ELSEIF (J.GE.2) THEN
                        NUM1 = 1.D0
                        DO I=1,2*(J-1)+A+B+2
                           NUM1 = NUM1*I
                        ...
                        dg%JACOBI(J+1,A+1,B+1,Q) = (COEFF1*POLY1+COEFF2*POLY2)&
                       /DEN
                     ENDIF
                  ENDDO
               ENDDO
...
!.....points and the edge midpoint
         ...
         DO Q=1,dg%NEGP(L)+1
            ...
            IF (Q.LT.(dg%NEGP(L)+1)) THEN
               IF (II.EQ.1) YGP =  dg%XEGP(Q,L)
               IF (II.EQ.2) YGP = -dg%XEGP(Q,L)
               IF (II.EQ.3) YGP = -1.D0
            ELSE
               IF (II.EQ.1) YGP =  0.D0
               IF (II.EQ.2) YGP =  0.D0
            ...
            DO I=0,P
               DO J=0,P-I
                  dg%PHI2(I+1,J+1,Q) = dg%JACOBI(I+1,1,1,Q)*((1.D0-YGP)/2.D0)**I&
         ...
         DO Q = 1,dg%NEGP(L)+1
            ...
            DO J = 0,P
               DO I = 0,J
                  JJ = J - I
                  IF (Q.LT.(dg%NEGP(L)+1)) THEN
                     dg%PHI_EDGE(M,Q,II,P) = dg%PHI2(I+1,JJ+1,Q)
                  ELSE
                     dg%PHI_MID(M,II,P) = dg%PHI2(I+1,JJ+1,Q)
                  ENDIF
                  dg%M_INV(M,P) = ((2.D0*I+1.D0)*(2.D0*JJ+2.D0*I+2.D0)/4.D0)
                  M = M+1
               ENDDO
            ENDDO
            IF (P.GT.1) THEN
               IF (Q.LT.(dg%NEGP(L)+1)) THEN
                  dg%PSI_CHECK(1,III) = -1.D0/2.D0*(XGP + YGP )
                  dg%PSI_CHECK(2,III) =  1.D0/2.D0*(XGP + 1.D0)
                  dg%PSI_CHECK(3,III) =  1.D0/2.D0*(YGP + 1.D0)
                  III = III + 1
               ENDIF
            ENDIF
         ENDDO
