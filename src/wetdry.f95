...
!.....Declare local variables
      ...
      REAL(SZ) DH_HAT(3)
      REAL(SZ) HT_HAT(dg_here%NCHECK(dg_here%ph)), HT_MODE(42)
      REAL(SZ) HT_NODE(dg_here%NCHECK(dg_here%ph))
      REAL(SZ) QX_HAT(3), QX_NODE(3), QY_HAT(3), QY_NODE(3)
      REAL(SZ) ZE_HAT(dg_here%NCHECK(dg_here%ph),dg_here%ph), ZE_NODE(dg_here%NCHECK(dg_here%ph),dg_here%ph)
...
!.......and count number of dry nodes
         ...
         HT_MIN = HUGE
         HT_MAX = ZERO
         DO I = 1,dg_here%NCHECK(dg_here%pa)
            !
            ZE_NODE(I,dg_here%pa) = dg_here%ZE(1,J,dg_here%IRK+1)
            DO K = 2,dg_here%DOFS(J)
               ZE_NODE(I,dg_here%pa) = ZE_NODE(I,dg_here%pa) + dg_here%PHI_CHECK(K,I,dg_here%pa)*dg_here%ZE(K,J,dg_here%IRK+1)
                                ! print*,dg_here%phi_check(k,i,dg_here%pa),k,i,dg_here%pa
            ENDDO
            HT_NODE(I) = ZE_NODE(I,dg_here%pa) + dg_here%DP_NODE(I,J,dg_here%pa)
                                ! print*,dg_here%DP_node(i,j,dg_here%pa), i,j
#ifdef SED_LAY   
            HT_NODE(I) = 0.D0
            do l=1,layers !notice we are summing over layers at nodes
               dg_here%DP_NODE(I,J,dg_here%pa) = dg_here%bed(1,J,dg_here%IRK+1,l)
               DO K = 2,dg_here%DOFS(J)
                  dg_here%DP_NODE(I,J,dg_here%pa) = dg_here%DP_NODE(I,J,dg_here%pa) + dg_here%PHI_CHECK(K,I,dg_here%pa)*dg_here%bed(K,J,dg_here%IRK+1,l)
               ENDDO
            enddo
#endif
            HT_NODE(I) = dg_here%DP_NODE(I,J,dg_here%pa) + ZE_NODE(I,dg_here%pa)
...
!.......Set "average" bathymetry based on p
         ...
         !DP_AVG = 1.D0/dg_here%ncheck(dg_here%pa) * (sum(dg_here%DP_NODE(:,J,dg_here%pa)))        
...
!...........Make no adjustments now
               ...
               ZE_HAT(:,dg_here%pa) = ZE_NODE(:,dg_here%pa)
...
!-----------------------------------------------------------------------
         ...
         ELSEIF (NDRYNODE.EQ.dg_here%NCHECK(dg_here%pa)) THEN
...
!.........water depth at all points = average water depth
            ...
            dg_here%ZE(1,J,dg_here%IRK+1) = HT_AVG - DP_AVG ! for dg_here%pa = 0, i.e FVM
            IF ( dg_here%pa > 0 ) THEN 
               ! Prevent water from becoming dangeously low by            !
               ! bumping the water depth back to ZERO, i.e. a small value !
               IF ( HT_AVG < ZERO ) dg_here%ZE(1,J,dg_here%IRK+1) = ZERO - DP_AVG 
            END IF
            dg_here%ZE(2,J,dg_here%IRK+1) = -dg_here%HB(2,J,1)
            dg_here%ZE(3,J,dg_here%IRK+1) = -dg_here%HB(3,J,1)
...
!            do i = 4,dg_here%dofh
!               
!               dg_here%ZE(i,J,dg_here%IRK+1) = 0.D0
!               dg_here%QX(i,J,dg_here%IRK+1) = 0.D0
!               dg_here%QY(i,J,dg_here%IRK+1) = 0.D0
!
!$$$#ifdef SED_LAY !is this more stable?
!$$$               do l=1,layers 
!$$$
!$$$                  dg_here%bed(i,J,dg_here%IRK+1,l) = 0.D0
!$$$
!$$$               enddo
!$$$#endif
!               
...
!...........water depth at all points = average water depth
               ...
               dg_here%ZE(:,J,dg_here%IRK+1) = 0.D0
               dg_here%ZE(1,J,dg_here%IRK+1) = HT_AVG - DP_AVG 
               IF ( dg_here%pa > 0 ) THEN
               ...
               dg_here%ZE(2,J,dg_here%IRK+1) = -dg_here%HB(2,J,1)
               dg_here%ZE(3,J,dg_here%IRK+1) = -dg_here%HB(3,J,1)
               ZE_HAT(:,dg_here%pa) = dg_here%ZE(1,J,dg_here%IRK+1)
...
!...........water depth and the same linear components
               ...
               IF (dg_here%pa.GT.1) THEN
                                !Notice the trick here.  Nodes = vertices
                                !since we NOW restrict to a linear regime
                  !c--- dw: should it be (?)
                  ! HT_MODE(2) = -(dg_here%ZE(2,J,1) + dg_here%HB(2,J,1))
                  ! HT_MODE(3) = -(dg_here%ZE(3,J,1) + dg_here%HB(3,J,1))
                  HT_MODE(1) = HT_AVG
                  HT_MODE(2) = -(dg_here%ZE(2,J,1) + dg_here%HB(2,J,1))
                  HT_MODE(3) = -(dg_here%ZE(3,J,1) + dg_here%HB(3,J,1))
                  HT_NODE = 0.D0
                  DO K = 1,3
                     HT_NODE(1) = HT_NODE(1) + dg_here%PHI_CORNER(K,1,dg_here%pa)*HT_MODE(K)
                     HT_NODE(2) = HT_NODE(2) + dg_here%PHI_CORNER(K,2,dg_here%pa)*HT_MODE(K)
                     HT_NODE(3) = HT_NODE(3) + dg_here%PHI_CORNER(K,3,dg_here%pa)*HT_MODE(K)
                  ENDDO
...
!...........fluxes on dry nodes, and count new # of dry nodes/vertices
               ...
               DO I = 1,3
                  ...
                  QX_NODE(I) = dg_here%QX(1,J,dg_here%IRK+1)
                  QY_NODE(I) = dg_here%QY(1,J,dg_here%IRK+1)
                  IF (HT_HAT(I).GT.HT_MAX) THEN
                     HT_MAX_I = I
                     HT_MAX = HT_HAT(I)
                  ENDIF
                  DO K = 2,dg_here%DOFS(J) !NOTE: We look at integrated value AT the vertex!
...
!.............Compute new linear modal dg_here%dofs for y-direction flux
                  ...
                  dg_here%QY(3,J,dg_here%IRK+1) = -0.5D0*QY_HAT(1) + 0.5D0*QY_HAT(2)
               ENDIF

!...........Compute new nodal surface elevations

               ZE_HAT(1:3,dg_here%pa) = HT_HAT(1:3) - dg_here%DP_NODE(1:3,J,dg_here%pa)

!...........Compute the new linear modal dg_here%dofs for the surface elevation

               dg_here%ZE(2,J,dg_here%IRK+1) = -dg_here%C16*(ZE_HAT(1,dg_here%pa) + ZE_HAT(2,dg_here%pa)) + dg_here%C13*ZE_HAT(3,dg_here%pa)
               dg_here%ZE(3,J,dg_here%IRK+1) = -0.5D0*ZE_HAT(1,dg_here%pa) + 0.5D0*ZE_HAT(2,dg_here%pa)

            ENDIF
            

!.........If applicable zero out higher dg_here%dofs
...
#ifdef TRACE
               dg_here%iota(i,J,dg_here%irk+1) = 0.D0
#endif
               
#ifdef CHEM         
               dg_here%iota(i,J,dg_here%irk+1) = 0.D0
               dg_here%iota2(i,j,dg_here%irk+1) = 0.D0
#endif

#ifdef DYNP
               dg_here%dynP(i,J,dg_here%irk+1) = 0.D0
#endif
