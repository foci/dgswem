...
!
!  mod history
!  v40.02mxxx - date - programmer - describe change 
!                    - mark change in code with  cinitials-mxxx 
!
!  v41.11 - 09/14/01 - rl - from 41.10 - added NWS = -2 capability
!  v41.09 - 06/30/01 - jw - from 41.08 - made minor mods as per vp version 41.05
!  v40.02m002 - 12/22 - jjw/vjp - Vic suggested this change to avoid compiler conflicts
...
!**************************************************************************
!
!  HOT START PROGRAM SETUP ROUTINE 
!
!**************************************************************************
!
      USE GLOBAL
...
!...
!......READ IN 2DDI HOT START INITIAL CONDITION OVER WHOLE DOMAIN
!......THIS FILE ALWAYS HAS A RECL=8 BECAUSE IT IS ASSUMED THAT THE HARMONIC
!......ANALYSIS IS ALWAYS DONE IN 64 BITS, EVEN ON A WORKSTATION
!...
...
!
!....SET POSITIONS IN BOUNDARY CONDITION, WIND AND OUTPUT FILES
!
        WRITE(16,1112)
        WRITE(16,1794)
...
!......INITIALLY, ZERO OUT THE NORMAL FLOW ON ALL BOUNDARIES
...
!....FIND THE PROPER PLACE IN THE APERIODIC ELEVATION SPECIFIED BOUNDARY CONDITION
!....FILE IF IT IS REQURIED.
...
!......FIND PROPER PLACE IN THE APERIODIC NORMAL FLOW BOUNDARY CONDITION FILE IF IT
!......IS REQUIRED
...
!......RESTART THE PERIODIC NORMAL FLOW BOUNDARY CONDITION
...
!...
!...RESTART SUPERCRITICAL OUTWARD NORMAL FLOW OVER SPECIFIED
...
!...
!...RESTART INWARD/OUTWARD NORMAL FLOW OVER SPECIFIED
!jjwm001 - modified/added the following 3 lines
!....INTERNAL BARRIERS AND FOR INTERNAL BARRIER BOUNDARIES
!....WITH CROSS BARRIER PIPES
!....THIS SECTION ONLY RESTARTS THE OVER BARRIER FLOW COMPONENT
!...
!jjwm001 - modified following line
        IF(NFLUXIB.EQ.1) THEN
          DO I=1,NVEL
!jjwm001 - modified following 2 lines
...
!.............RESET INFORMATION FOR K-1 TIME LEVEL
              ...
              ENDIF
1998          CONTINUE
!.............RESET INFORMATION FOR K TIME LEVEL
...
!...
!...RESTART INWARD/OUTWARD NORMAL FLOW OVER SPECIFIED
!....INTERNAL BARRIERS WITH CROSS BARRIER PIPES
!....THIS SECTION RESTARTS THE PIPE FLOW COMPONENT
!....NOTE THAT PIPE FLOW COMPONENT IS ADDED INTO BARRIER FLOW COMPONENT
...
!......RESTART WIND AND PRESSURE INFORMATION
 ...
 2223     CALL NWS3GET( X, Y, SLAM, SFEA, WVNX2, WVNY2, IWTIME, IWYR,&
        ...
        IF(NWS.EQ.6) THEN
          OPEN(22,FILE=DIRNAME//'/'//'fort.22')
!   The following 3 lines are a hardwire to allow a non standard met file to be read in at 
!   time zero in a hot start.  They should be eliminated or commented out for normal operation
!         OPEN(199,FILE=DIRNAME//'/'//'fort.199')
!         READ(199,*) (NHG,PRN1(I),WVNX1(I),WVNY1(I),I=1,NP)
!         CLOSE(199)
!   The following CALL statement should be uncommented for normal operation
...
!ek added NWS = 12 and -12 from version 46
...
!......RESTART THE WAVE RADIATION STRESS
...
!     &                                                  WAVE_D2,NP)
       ...
       if (CTIP) then
!jromo 11-01-00  Initialize TIP2 for HOTSTART
          DO I=1,NP
             TIP2(I)=0.0
          END DO
!TIP  LINES TO USE TIDAL POTENTIAL FORCING
       IF(NTIP.GE.1) THEN
         DO J=1,NTIF
           IF(PERT(J).EQ.0.) THEN
             NCYC=0
...
!...
!....SET UP TO RESTART TIMESERIES OUTPUT FILES
!....
!...
...
!...
!....IF RESTARTING THE ELEVATION STATION OUTPUT FILE, GO TO THE PROPER PLACE
!....IN THE FILE.  OTHERWISE ZERO OUT NSCOUE.
!...
...
!...
!....GO TO THE PROPER PLACE IN THE VELOCITY STATION OUTPUT FILE
!...
...
!...
!....GO TO THE PROPER PLACE IN THE CONCENTRATION STATION OUTPUT FILE
!...
 ...
 1045     FORMAT(//,' A NEW CONCENTRATION STATION FILE WILL BE STARTED')
        ...
        IF(NOUTC.EQ.2) THEN
          OPEN(81,FILE=DIRNAME//'/'//'fort.81',&
          ACCESS='DIRECT',RECL=NBYTE)
!revisit          WRITE(81,REC=ITEMPSTP+1) NTRSPC   ! ALLOW ADDITIONAL OUTPUT DATA TO BE WRITTEN
          CLOSE(81)                         ! DO THIS TO FLUSH THE WRITE BUFFER
          OPEN(81,FILE=DIRNAME//'/'//'fort.81',&
         ACCESS='DIRECT',RECL=NBYTE)
          ENDIF
...
!...
!....GO TO THE PROPER PLACE IN THE METEOROLOGICAL STATION OUTPUT FILE
!...
 ...
 1039    FORMAT(//,' A NEW METEOROLOGICAL STATION FILE WILL BE STARTED')
        ...
        IF(NOUTM.EQ.2) THEN
          ...
          OPEN(72,FILE=DIRNAME//'/'//'fort.72',&
          ACCESS='DIRECT',RECL=NBYTE)
!revisit          WRITE(71,REC=ITEMPSTP+1) NTRSPM   ! ALLOW ADDITIONAL OUTPUT DATA TO BE WRITTEN
!revisit          WRITE(72,REC=ITMEPSTP+1) NTRSPM
          CLOSE(71)                         ! DO THIS TO FLUSH THE WRITE BUFFER
          CLOSE(72)
          OPEN(71,FILE=DIRNAME//'/'//'fort.71',&
         ACCESS='DIRECT',RECL=NBYTE)
...
!...
!....GO TO THE PROPER PLACE IN THE GLOBAL ELEVATION OUTPUT FILE
!...
...
!...
!....GO TO THE PROPER PLACE IN THE GLOBAL VELOCITY OUTPUT FILE
!...
...
!...
!....GO TO THE PROPER PLACE IN THE GLOBAL CONCENTRATION OUTPUT FILE
!...
...
!...
!....GO TO THE PROPER PLACE IN THE GLOBAL METEOROLOGICAL OUTPUT FILES
!...
...
!..Read in Means and Squares
       ...
       if(C3DVS) then
!3DVS..UNCOMMENT THE FOLLOWING LINES TO RUN THE CODE IN 3D VS MODE.
!3DVS..COMMENT OUT THE FOLLOWING LINES TO RUN THE CODE IN 2DDI OR 3D DSS MODE.
!3DVS..
!     ! CALL VSSTUP(DT,STATIM,NBYTE,RUNDES,RUNID,AGRID,NT)
!3DVS..
!3DVS..END OF 3D VS STATEMENTS (MORE FOLLOW BELOW)
!3DVS..
       end if
       ...
       if(C3DDSS)  then
!3DDSS.UNCOMMENT THE FOLLOWING LINES TO RUN THE CODE IN 3D DSS MODE
!3DDSS.COMMENT OUT THE FOLLOWING LINES TO RUN THE CODE IN 2DDI OR 3D VS MODE.
!3DDSS.
!     ! CALL DSSSTUP(DT,STATIM,NBYTE,RUNDES,RUNID,AGRID,NT)
!3DDSS.
!3DDSS.END OF 3D DSS STATEMENTS (MORE FOLLOW BELOW)
!3DDSS.
       end if
...
!.....Adjust Morphology timestep counter (defunct)
